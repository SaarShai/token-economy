...
13. Fresh session starts in plan mode, thinks step by step, and creates a robust plan before executing.
...
Then check `./te context host-controls --agent auto` and `./te context fresh-command --agent auto --handoff <handoff-file>`. If running in Codex and App Server is available, use `./te context codex-fresh-thread --handoff <handoff-file> --execute` to start a persistent fresh successor thread in this project; pass `--model <available-model>` if needed. Treat the App Server path as successful only if it reports `ok: true`, `thread_persistent: true`, `thread_turns_empty: true`, `assistant_responded: true`, `thread_idle: true`, and ideally `listed_after_start: true`. Do not assume you can execute host slash commands from your own response. Prefer a fresh successor session/process if direct clear is unavailable. Tell me the exact command I or the host should run and exactly what to paste next. Do not load anything else until retrieval proves relevance.
