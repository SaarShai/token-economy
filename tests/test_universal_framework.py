...
""",
    ...
    def test_docs_audit_targets_startup_surface_only(self):
        ...
        self.assertIn("<OLD_PROJECT_ROOT>", full_summ)
        self.assertIn("<OBSIDIAN_WIKI_ROOT>", full_summ)
        self.assertIn("<OUTPUT_FULL_SUMM>", full_summ)
        self.assertIn("Obsidian/wiki pages", full_summ)
        self.assertIn("LOCAL SECRET MATERIAL - DO NOT COMMIT", full_summ)
        self.assertIn("Raw API keys/secrets are explicitly authorized", full_summ)
        self.assertIn("context rot", full_summ)
        self.assertIn("Write exactly one output file", full_summ)
        ...
        self.assertIn("Do not commit `<FULL_SUMM_PATH>`, `.env`, raw secret files, credentials", import_full_summ)
