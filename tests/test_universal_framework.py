...
""",
    ...
    def test_docs_audit_targets_startup_surface_only(self):
        ...
        self.assertIn("Do not stop merely because the local `./te` lacks the new subcommands", summ)
        self.assertIn("codex fork --last", summ)
        manual_summ = (REPO / "prompts/summ-codex-manual.md").read_text(encoding="utf-8")
        self.assertIn("self-contained App Server compaction fallback", manual_summ)
        self.assertIn("codex app-server --help", manual_summ)
        self.assertIn("thread/compact/start", manual_summ)
        self.assertIn("Do not stop merely because local `./te` is old", manual_summ)
